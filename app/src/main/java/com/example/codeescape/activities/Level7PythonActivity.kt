...
class Level7PythonActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_level7_python)

        playerPuerta = MediaPlayer.create(this, R.raw.sonido_puerta)
        GameManager.reiniciarVidas()

        val btnComp: Button = findViewById(R.id.btnPyComp)
        val btnLambda: Button = findViewById(R.id.btnPyLambda)
        val btnDoor: Button = findViewById(R.id.btnPyDoor7)
        val btnBack: Button = findViewById(R.id.btnBackPy7)
        ...
        btnLambda.setOnClickListener {
            if (!sublevel1Done) {
                Toast.makeText(this, "⚠️ Primero domina la sintaxis final", Toast.LENGTH_SHORT).show()
            } else {
                val intent = Intent(this, SubLevelPythonActivity::class.java)
        ...
        btnDoor.setOnClickListener {
            when {
                !sublevel1Done || !sublevel2Done -> {
                    Toast.makeText(this, "🔒 El sistema requiere optimización final", Toast.LENGTH_SHORT).show()
                }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (resultCode == RESULT_OK) {
            when (requestCode) {
                1 -> { sublevel1Done = true; actualizarProgreso(); Toast.makeText(this, "✅ Sintaxis optimizada", Toast.LENGTH_SHORT).show() }
                2 -> { sublevel2Done = true; actualizarProgreso(); Toast.makeText(this, "✅ Lambda funcional", Toast.LENGTH_SHORT).show() }
    ...
    private fun actualizarProgreso() {
        val tvProgress: TextView = findViewById(R.id.tvPyProgress7)
    ...
    private fun mostrarVictoria() {
        ...
        tvDoorStatus.text = "MAESTRO"
        tvDoorStatus.setTextColor(android.graphics.Color.parseColor("#00FF41"))

        val prefs = getSharedPreferences("CodeEscape", MODE_PRIVATE)
        prefs.edit().putBoolean("py_nivel7_completo", true).apply()
        GameManager.nivelCompletado()
        GameManager.guardar(this)

        AlertDialog.Builder(this)
            .setTitle("🎉 ¡MAESTRÍA ALCANZADA!")
            .setMessage("Has completado todos los niveles de Python.\n\n¡Eres un Maestro de la Serpiente!")
