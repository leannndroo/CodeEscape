...
class Level5PythonActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_level5_python)

        playerPuerta = MediaPlayer.create(this, R.raw.sonido_puerta)
        GameManager.reiniciarVidas()

        val btnHereda: Button = findViewById(R.id.btnPyHereda)
        val btnSuper: Button = findViewById(R.id.btnPySuper)
        val btnDoor: Button = findViewById(R.id.btnPyDoor5)
        val btnBack: Button = findViewById(R.id.btnBackPy5)
        val btnVolume: Button = findViewById(R.id.btnVolume)

        btnVolume.setOnClickListener { VolumeDialog.show(this) }

        btnHereda.setOnClickListener {
        ...
        btnSuper.setOnClickListener {
            if (!sublevel1Done) {
                Toast.makeText(this, "⚠️ Primero define la herencia", Toast.LENGTH_SHORT).show()
            } else {
                val intent = Intent(this, SubLevelPythonActivity::class.java)
        ...
        btnDoor.setOnClickListener {
            when {
                !sublevel1Done || !sublevel2Done -> {
                    Toast.makeText(this, "🔒 Falta definir el comportamiento", Toast.LENGTH_SHORT).show()
                }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (resultCode == RESULT_OK) {
            when (requestCode) {
                1 -> { sublevel1Done = true; actualizarProgreso(); Toast.makeText(this, "✅ Herencia establecida", Toast.LENGTH_SHORT).show() }
                2 -> { sublevel2Done = true; actualizarProgreso(); Toast.makeText(this, "✅ Llamada al padre lista", Toast.LENGTH_SHORT).show() }
    ...
    private fun actualizarProgreso() {
        val tvProgress: TextView = findViewById(R.id.tvPyProgress5)
